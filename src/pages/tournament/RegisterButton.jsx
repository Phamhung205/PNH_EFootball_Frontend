...
export default function RegisterButton({ tournament, user, darkMode = true, language = 'vi', onOpenChat, chatEnabled = false }) {
  ...
  return (
    <div className="inline-flex flex-col items-start gap-2">
      ...
      {/* Da dang ky + admin DA BAT chat -> hien nut vao box chat */}
      {registered && onOpenChat && chatEnabled && (
      ...
      {registered && !chatEnabled && (
        <p className={`text-xs ${dm ? 'text-slate-400' : 'text-slate-500'}`}>
          ✓ Bạn đã đăng ký. Chờ ban tổ chức mở box chat.
        </p>
      )}
