...
import { Trophy, Users, Calendar, Swords, Layers, CheckCircle2, AlertCircle } from 'lucide-react';

const FORMAT_OPTIONS = [
  {
    id: 'group-stage',
    label: 'Đấu Bảng',
    desc: 'Chia thành nhiều bảng, top đội vào vòng tiếp',
    icon: Layers,
  },
  {
    id: 'knockout',
    label: 'Loại Trực Tiếp',
    desc: 'Thua là bị loại ngay',
    icon: Swords,
  },
  {
    id: 'league',
    label: 'Giải Đường Dài',
    desc: 'Tất cả đấu với nhau theo lượt',
    icon: Calendar,
  },
  {
    id: 'hybrid',
    label: 'Hỗn Hợp',
    desc: 'Bảng rồi đấu loại trực tiếp',
    icon: Trophy,
  },
];
      ...
      <div className={`w-full max-w-2xl rounded-2xl ${card} p-6 md:p-8 flex flex-col gap-6`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className={`text-sm ${darkMode ? 'text-white/50' : 'text-slate-400'}`}>
              Điền thông tin để khởi tạo giải đấu
            </p>
          </div>
        </div>
          ...
          <button
            ...
            className={`w-full py-3 rounded-xl font-bold text-white text-sm transition-all duration-200 flex items-center justify-center gap-2 ${
              success
                ? 'bg-emerald-500'
                : submitting
                ? 'bg-gradient-to-r from-emerald-500 to-cyan-500 opacity-70 cursor-not-allowed'
